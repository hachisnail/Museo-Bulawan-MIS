...
const Artifact = () => {
  ...
  // Fetch artifacts using our new API endpoint with filtering
  const fetchArtifacts = async () => {
    ...
    try {
      ...
      if (filterStatus !== 'all') {
        params.append('display_status', filterStatus)
      }

      if (filterType !== 'all') {
        params.append('artifact_type', filterType)
      }
      ...
      if (activeTab === 'acquired') {
        // For acquired tab, show only donated and purchased artifacts
        params.append('accession_type', 'donated,purchased')
      } else if (activeTab === 'borrowing') {
        // For borrowing tab, show only lend artifacts
        params.append('accession_type', 'lend')
      }

      const response = await axios.get(
        `${API_URL}/api/auth/artifact?${params.toString()}`,
        { withCredentials: true }
      )

      setArtifacts(response.data.artifacts || [])
      setError(null)
      showToast(`${response.data.artifacts?.length || 0} artifacts loaded`, 'info')
    } catch (err) {
  ...
  // Fetch artifacts when filters change
  useEffect(() => {
    fetchArtifacts()
  }, [
    activeTab,
  ...
  const openArtifactView = async (artifact) => {
    try {
      // Fetch the complete artifact data by ID
      const response = await axios.get(
        `${API_URL}/api/auth/artifact/${artifact.id}`,
        { withCredentials: true }
      );

      // Set the complete artifact data and open the modal
      setViewingArtifact(response.data);
      setIsViewArtifactOpen(true);
    } catch (error) {
      console.error('Error fetching artifact details:', error);
      showToast('Could not load artifact details', 'error');
    }
  };
                  ...
                  <>
                    ...
                    <div className='w-full h-full flex flex-col xl:flex-row gap-y-5 xl:gap-y-0 xl:gap-x-5 border-t-1 pt-5'>
                        ...
                        <div className='w-full h-full flex flex-col gap-y-[5rem]'>
                          ...
                          <div className='w-full h-auto flex flex-col gap-y-7'>
                            ...
                            <div className='w-full h-fit flex justify-between items-center'>
                              ...
                              <div className='w-[5rem] h-[2rem] flex items-center bg-[#D4DBFF] rounded-md justify-center'>
                                <span className='text-2xl font-semibold'>
                                  {activeTab === 'acquired' ? (artifacts.length || 0) : artifacts.filter(a =>
                                    a.accession_type === 'donated' || a.accession_type === 'purchased').length}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className='w-full h-fit flex justify-between items-center'>
                              ...
                              <div className='w-[5rem] h-[2rem] flex items-center bg-[#D4DBFF] rounded-md justify-center'>
                                <span className='text-2xl font-semibold'>
                                  {activeTab === 'borrowing' ? (artifacts.length || 0) : artifacts.filter(a =>
                                    a.accession_type === 'lend').length}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className='w-full h-fit flex justify-between items-center'>
                              ...
                              <div className='w-[5rem] h-[2rem] flex items-center bg-[#D4DBFF] rounded-md justify-center'>
                                <span className='text-2xl font-semibold'>
                                  {filterStatus === 'displayed' ? (artifacts.length || 0) : artifacts.filter(a =>
                                    a.display_status === 'displayed').length}
                                </span>
                              </div>
                            </div>
                      ...
                      <div className=' w-full h-full flex flex-col gap-y-7 overflow-x-scroll overflow-y-scroll'>
                        ...
                        <div className="w-full min-w-[94rem] h-full flex flex-col border-t border-t-gray-400 overflow-y-auto">
                          {/* Loading state */}
                          {loading && (
                          ...
                          {/* Artifact rows */}
                          {!loading && !error && artifacts.length > 0 && artifacts.map((artifact) => (
                            <div
                              key={artifact.id}
                            ...
                            >
                              ...
                              <div className="px-4 pt-1 pb-3 border-b border-gray-400 col-span-2">
                                <span>{safeGet(artifact, 'name', 'Untitled Artifact')}</span>
                              </div>
                              <div className="px-4 pt-1 pb-3 border-b border-gray-400 col-span-1">
